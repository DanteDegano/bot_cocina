...
if (window.ChefBotLoaded) {
  ...
  function createWidget() {
    ...
    // Widget HTML
    document.body.innerHTML = '<div style="width:100vw;height:100vh;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:flex;align-items:center;justify-content:center;position:fixed;top:0;left:0;z-index:9999;"><div style="background:white;border-radius:20px;padding:40px;text-align:center;box-shadow:0 10px 30px rgba(0,0,0,0.3);max-width:500px;width:90%;"><div style="width:80px;height:80px;background:#FFD700;border-radius:50%;margin:0 auto 20px auto;display:flex;align-items:center;justify-content:center;font-size:40px;">👨‍🍳</div><h1 style="color:#333;margin-bottom:10px;">ChefBot</h1><p style="color:#666;margin-bottom:20px;">Tu asistente personal de cocina</p><div id="status" style="padding:15px;background:#d4edda;border-radius:10px;margin-bottom:20px;color:#155724;">✅ ¡Listo para ayudarte!</div><input type="text" id="chatInput" placeholder="Pregúntame sobre cocina..." style="width:100%;padding:15px;border:2px solid #e0e0e0;border-radius:25px;font-size:16px;margin-bottom:15px;box-sizing:border-box;"><button onclick="sendMessage()" style="width:100%;padding:15px;background:#FFD700;border:none;border-radius:25px;font-size:16px;font-weight:bold;cursor:pointer;">Enviar Pregunta</button><div id="response" style="margin-top:20px;padding:15px;background:#f8f9fa;border-radius:10px;text-align:left;min-height:60px;display:none;"></div></div></div>';
    ...
    window.sendMessage = function() {
      ...
      setTimeout(() => {
        response.innerHTML = '<strong>🍳 ChefBot responde:</strong><br><br>¡Excelente pregunta sobre "' + message + '"!<br><br>Como tu asistente de cocina, te puedo ayudar con recetas, técnicas culinarias, sustitutos de ingredientes y consejos profesionales.<br><br><em>* Integración con Gemini AI próximamente *</em>';
        input.value = '';
      }, 1500);
    };
